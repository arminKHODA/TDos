...
def process_single_image(args):
    ...
    try:
        ...
        (R, G, B) = [np.fromstring(file.channel(c, FLOAT), dtype=np.float32) for c in 'RGB']

        # Normalize so that brightest pixel is 1.0
        img = np.zeros((size[1], size[0], 3), dtype=np.float32)
        img[:, :, 0] = np.reshape(R, (size[1], size[0]))
        img[:, :, 1] = np.reshape(G, (size[1], size[0]))
        img[:, :, 2] = np.reshape(B, (size[1], size[0]))
        ...
        img_pil.save(os.path.join(folder, "jpg_result", new_name), "JPEG")
